...
//paymentData = [{ date: '2024-01-01', payments: 1500 }, ...]
  ...
  tailwindtheme: {
    "title": "Advanced Tailwind Theme Preset + Custom Design Tokens",
    "subtitle": "Building a Scalable Design System with Tailwind, CSS Variables & Utilities",
    "description": "Configured a fully customizable Tailwind preset using CSS variables, design tokens, custom fonts, color system, scrollbar styling, extended breakpoints, and reusable utilities like clsx and tailwind-merge. This setup acts as a scalable design foundation across large React projects.",

    "problem": "The project required a scalable, brand-consistent UI theme shared across many modules. Default Tailwind colors and spacing were not aligned with Curebay’s visual identity. We needed a theme preset: custom colors, typography, scrollbar styling, responsive breakpoints, reusable cn() utility, and a theme-based Button component.",

    "installation": {
      "code": "npm install tailwindcss @tailwindcss/line-clamp tailwind-scrollbar tailwind-merge clsx",
      "language": "bash",
      "filePath": "Install Tailwind & Utilities"
    },
  ...
  mockservice: {
    ...
    "subtitle": "Building Full Product UI Without Backend APIs Using Mocked REST Endpoints",
    "description": "Implemented Mock Service Worker (MSW) to simulate REST APIs before backend development began. This enabled the entire UI of the Curebay project to be developed, tested, and validated with real-world API flows without needing any backend ready. Later, the same mock API contracts were shared with backend developers who implemented the server to match the exact schema.",

    "problem": "The backend team was not ready with APIs, but the UI development timeline was tight. I needed a way to build UI screens, handle async flows, test error states, validate forms, create realistic table/filter/pagination UI, and preview the full user experience — all without any real API. MSW enabled contract-first development with full reliability.",

    "installation": {
      "filePath": "Install MSW",
      "language": "bash",
  ...
  nginx: {
    "title": "End-to-End Code Deployment to Production",
    "subtitle": "From Local Development to Production Using Git Branching and CI/CD Pipelines",
    "description": "Implemented a full deployment workflow that takes code from a developer’s local machine all the way to a production environment using feature branches, pull requests, and CI/CD pipelines. The process covers planning, development, automated build and test, deployments across dev, staging, and production environments, and post-deployment monitoring to ensure reliability and rapid feedback loops.[web:5]",

    "problem": "Multiple developers needed a safe, repeatable way to ship features to production without breaking the live application. Direct commits to the main branch caused unstable releases, missing reviews, and no clear separation between dev, staging, and production environments. A structured pipeline was needed so that every change could be tracked, tested, and promoted gradually with minimal risk to end users.[web:5]",
  ...
  formio: {
    "title": "Dynamic State-Wise Workflow Engine Using Form.io",
    "subtitle": "Schema-Driven Forms for All States of India Without Redeployment",
    "description": "Implemented a fully dynamic Form.io architecture to handle different telecom infrastructure approvals workflows for every state in India. Instead of hardcoding UI forms for 36 states/UTs, Form.io allowed the entire form experience—fields, validation, conditional logic, API data sources, and workflow rules—to be driven by JSON schema. Any state-level change required zero redeployment.",

    "problem": "India's state-wise telecom infrastructure approvals workflow required different fields, rules, document requirements, dropdown values, and conditional visibility logic. Hardcoding forms for each state or creating multiple React components would have led to massive maintenance effort and redeployment for every change. The team needed a schema-driven, backend-configurable, real-time modifiable form system.",

    "installation": {
      "filePath": "Install Form.io",
      "language": "bash",
      "code": "npm install @formio/react @formio/js"
    },

    // "demo": "<div class='rounded-xl border shadow p-4 bg-card'><p>✨ Below is a preview of how state-wise forms dynamically change without deploying:</p><img src='/demos/formio-dynamic-state.gif' class='rounded border mt-2'/></div>",
  ...
  otpform: {
    ...
    "subtitle": "Secure Email OTP Flow using Zod, React Hook Form & Countdown Timer",
    "description": "Implemented a production-grade OTP verification flow with a 15-minute countdown timer, resend lockout mechanism, Zod validation, React Hook Form, and RTK Query API integration. This ensured secure login, prevented abuse of resend API, and created a smooth authentication UX.",

    "problem": "The project required a secure OTP login flow with strict validation, countdown timer, resend restrictions, and API integration. The OTP screen needed to handle incorrect OTPs, rate limit OTP resend, auto-navigate after success, and show proper error states, all without leaking implementation details or exposing vulnerabilities.",
    "demo": "<img src='/images/casestudy/otpform.png' class='rounded-lg border shadow-sm w-full' />",

    "installation": {
      "filePath": "Install Required Packages",
      "language": "bash",
      "code": "npm install react-hook-form @hookform/resolvers zod react-countdown-hook react-toastify\nnpm install @reduxjs/toolkit react-redux\n"
    },
